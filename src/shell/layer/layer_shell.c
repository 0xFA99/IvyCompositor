#include "core/fwd.h"
#include "core/types.h"
#include "core/server.h"
#include "shell/layer/layer_shell.h"
#include "shell/layer/layer_surface.h"

#define IVY_LAYER_SHELL_VERSION 4

void Ivy_LayerShell_Init(IvyLayerShell *layer_shell)
{
    IVY_ASSERT(layer_shell != NULL, "[ERROR] IvyLayerShell is NULL!");

    IvyServer *server = wl_container_of(layer_shell, server, shell.layer_shell);

    layer_shell->wlr_layer_shell = wlr_layer_shell_v1_create(server->core.wl_display, IVY_LAYER_SHELL_VERSION);
    IVY_CHECK(layer_shell->wlr_layer_shell != NULL, "[WARNING] Failed to create wlr_layer_shell!");

    Ivy_LayerSurfaceManager_Init(&layer_shell->surface_manager);
}


void Ivy_LayerShell_Destroy(IvyLayerShell *layer_shell)
{
    IVY_ASSERT(layer_shell != NULL, "[WARNING] IvyLayerShell is NULL!");

    wl_list_remove(&layer_shell->surface_manager.new_surface.link);
}